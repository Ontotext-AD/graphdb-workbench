define(['angular/core/services'],
    function () {
        ...
        importCtrl.controller('CommonCtrl', ['$scope', '$http', 'toastr', '$interval', '$timeout', '$repositories', '$modal', '$filter', '$rootScope', '$jwtAuth', '$location', function ($scope, $http, toastr, $interval, $timeout, $repositories, $modal, $filter, $rootScope, $jwtAuth, $location) {
            $scope.files = [];
            $scope.fileChecked = {};
...

            {
                ...
                $scope.textFileFormatsHuman = _.reduce(_.filter(basics, function (el) { return el !== '.brf'}),
                    function (el, all) {
                        return el + ' ' + all;
                    });
            }
        ...
        importCtrl.controller('UploadCtrl', ['$scope', 'Upload', '$http', 'toastr', '$controller', '$rootScope', '$modal', function ($scope, Upload, $http, toastr, $controller, $rootScope, $modal) {
            ...
            $scope.pasteData = function(file) {
                ...
                var modalInstance = $modal.open({
                    ...
                    controller: 'TextCtrl',
                    resolve: {
                        text: function() { return file ? file.data : '' },
                        format: function() { return file ? file.format : 'text/turtle' }
                    }
                });
