...
export class AdditionalExtractionMethodsFormModel {
     ...
     * @return {number}
     */
    findAdditionalExtractionMethodIndex(method) {
        return this._additionalExtractionMethods.findIndex((additionalExtractionMethod) => additionalExtractionMethod.method === method);
    }
    ...
    setAdditionalExtractionMethod(additionalExtractionMethod) {
        if (Array.isArray(additionalExtractionMethod)) {
            additionalExtractionMethod.forEach(additionalExtractionMethod => {
                const index = this.findAdditionalExtractionMethodIndex(additionalExtractionMethod._method);
                if (index !== -1) {
                    const existingMethod = this._additionalExtractionMethods[index];
