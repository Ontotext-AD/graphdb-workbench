...
function sparqlTabDirective($rootScope, LocalStorageAdapter, LSKeys, ModalService) {
    ...
    return {
        ...
        template: '<a class="nav-link" role="tab" data-toggle="tab" blur="submit" editable-text="tab.name" e-form="editCurrentlySelectedOnly" ng-click="selectThisTab($event)" ng-dblclick="editCurrentTab()" ><span ng-class="{\'text-muted\': !tab.name}"><span ng-if="tab.name">{{tab.name}}</span><span ng-if="!tab.name">{{\'sparql.tab.directive.unnamed.tab.title\' | translate}}</span></span><button type="button" ng-click="deleteTab($event)" class="btn btn-link btn-sm secondary delete-sparql-tab-btn" title="Delete tab"><i class="icon-close"></i></button></a>',
        replace: true,
