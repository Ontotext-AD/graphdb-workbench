...
function GraphsVisualizationsCtrl(
    ...
    const loadGraphForQuery = (queryString, sameAsParam, inferredParam) => {
        ...
        GraphDataRestService.updateGraph({
            ...
            // Node draw will turn off the loader
    ...
    const NODE_COLORS_VARIABLES = [
        '--gw-foreground-visual-graph-categorical-01',
        '--gw-foreground-visual-graph-categorical-02',
        '--gw-foreground-visual-graph-categorical-03',
        '--gw-foreground-visual-graph-categorical-04',
        '--gw-foreground-visual-graph-categorical-05',
        '--gw-foreground-visual-graph-categorical-06',
        '--gw-foreground-visual-graph-categorical-07',
        '--gw-foreground-visual-graph-categorical-08',
        '--gw-foreground-visual-graph-categorical-09',
        '--gw-foreground-visual-graph-categorical-10',
        '--gw-foreground-visual-graph-categorical-11',
        '--gw-foreground-visual-graph-categorical-12',
        '--gw-foreground-visual-graph-categorical-13',
        '--gw-foreground-visual-graph-categorical-14',
        '--gw-foreground-visual-graph-categorical-15',
        '--gw-foreground-visual-graph-categorical-16',
        '--gw-foreground-visual-graph-categorical-17',
        '--gw-foreground-visual-graph-categorical-18',
        '--gw-foreground-visual-graph-categorical-19',
        '--gw-foreground-visual-graph-categorical-20',
    ];
    ...
    function draw(resetRootNode) {
        ...
        const rightClickHandler = function(event, d) {
            if (event.shiftKey) {
                // Do nothing with the shift key, use to access the context menu
                return;
            }
    ...
    function renderGraphFromResponse(response, d, isStartNode) {
        ...
        if (newNodes.length === 0) {
            ...
            const connectionsCount = nodesFromLinks.length > 0 ? nodesFromLinks.length - 1 : 0;
            if (isStartNode) {
                toastr.info($translate.instant('graphexplore.error.node.connections', {count: connectionsCount}));
        ...
        this.initIcons = function(node, parentNode) {
            ...
            if (node.isExpanded) {
                ...
                this.collapseIcon.append("path")
                    ...
                    .style("fill", "var(--gw-primary-base)");

                this.collapseIcon.on("click", function() {
                    collapseNode(node);
                }).on('mouseover', function(event) {
                    actionsTip.show(event.currentTarget.getAttribute("name"), event);
                    $timeout.cancel(showNodeTipAndIconsTimer);
                    $timeout.cancel(removeIconsTimer);
    ...
    function linkActions(event, d) {
        ...
        if (d.predicates.length === 1 && graph.tripleNodes.has(linkId)) {
            ...
            if (clickedNode(tripleNode, event.currentTarget, event.button)) {
                const nodeCopy = {};
                nodeCopy.iri = tripleNode.iri;
                nodeCopy.pred = getShortPredicate(tripleNode.iri.split(' ')[1]);
                applyElementsStyleChanges(nodeCopy);
