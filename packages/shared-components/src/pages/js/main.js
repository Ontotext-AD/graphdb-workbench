...
const updateNamespaces = (rawNamespaces) => {
  testContext.updateNamespaces(rawNamespaces);
};

const setGdbToken = (token) => {
  testContext.setGdbToken(token);
};

// Make functions available globally explicitly, so lint knows they are used
window.updateLicense = updateLicense;
window.loadProductInfo = loadProductInfo;
window.addToastr = addToastr;
window.loadRepositories = loadRepositories;
window.setAuthUser = setAuthUser;
window.setSecurityConfig = setSecurityConfig;
window.setMarvelRepo = setMarvelRepo;
window.updateIsLoadingActiveRepositoryLocation = updateIsLoadingActiveRepositoryLocation;
window.setAutocomplete = setAutocomplete;
window.updateNamespaces = updateNamespaces;
window.setGdbToken = setGdbToken;
