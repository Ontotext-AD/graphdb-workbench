import {
  ...
  SearchButtonConfig,
  ServiceProvider,
  SubscriptionList, SUGGESTION_SELECTED_EVENT, SuggestionSelectedPayload
    ...
    return new SearchButtonConfig({
      ...
      buttons: [
        ...
        this.createSearchButton(
          ...
          () => {
            this.redirectUrl = 'graphs-visualizations';
          },
          false
        )
      ]
    ...
    this.subscriptions.add(
      this.eventService.subscribe<SuggestionSelectedPayload>(SUGGESTION_SELECTED_EVENT, (payload) => {
        if (payload.getContext() === this.RDF_CONTEXT) {
          const fullUrl = `${this.redirectUrl}?uri=${encodeURIComponent(payload.getSuggestion().getValue())}`;
