...
import type {StepDescription} from '../../../../services/guides/step-description';
...
export class InteractiveGuide extends Model<InteractiveGuide>{
  ...
  private readonly options: GuideOptions;
  ...
  /**
   * Flattens the guide's complex steps into an array of fully resolved {@link StepDescription} objects
   ...
   */
  toStepsDescriptions(guidePlugins: GuidePlugin[], services: unknown): StepDescription[] {
    return this.getSteps().flatMap((step) => this._getSteps(step, this.getOptions(), guidePlugins, services));
  }

  private _getSteps(complexStep: GuideStep | GuideStep[], parentOptions: GuideOptions, guidePlugins: GuidePlugin[], services: unknown): StepDescription[] {
    ...
    const predefinedStepDescription = guidePlugins.find((predefinedStep) => predefinedStep.guideBlockName === complexStep.guideBlockName);

    if (!predefinedStepDescription) {
      return [];
    }

    const options = {...(predefinedStepDescription.options ?? {}), ...complexStep.options, ...parentOptions}; //NOSONAR

    let steps: StepDescription[] = [];

    if (predefinedStepDescription.getSteps) {
      steps = steps.concat(this._getSteps(
        ObjectUtil.deepCopy(predefinedStepDescription.getSteps(options, services)) as GuideStep[],
        parentOptions, guidePlugins, services
      ));
    } else if (predefinedStepDescription.getStep) {
      steps.push(ObjectUtil.deepCopy(predefinedStepDescription.getStep(options, services)) as StepDescription);
    } else {
      // All steps should have either getStep or getSteps method implemented. If not, it's an error in the plugin implementation.
      throw new GuideStepNotCorrectlyImplementedError(predefinedStepDescription.guideBlockName);
    }
