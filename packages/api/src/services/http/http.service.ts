...
export class HttpService {
  ...
  put<T>(url: string, options?: HttpOptionsTypedResponse): Promise<HttpResponseResult<T>> {
  ...
  patch<T>(url: string, options?: HttpOptionsTypedResponse): Promise<HttpResponseResult<T>> {
    const {params, body, headers, responseType = 'body'} = options ?? {};
    if (responseType === 'response') {
  ...
  delete<T>(url: string, options?: HttpOptionsTypedResponse): Promise<HttpResponseResult<T>> {
    const {params, headers, responseType = 'body'} = options ?? {};
    if (responseType === 'response') {
      ...
      .then(async (response) => {
        ...
        if (!response.ok) {
          ...
          const errorResponse = new HttpErrorResponse({
            ...
            originalResponse: response,
            data: data as HttpResponseData<T>
          });
