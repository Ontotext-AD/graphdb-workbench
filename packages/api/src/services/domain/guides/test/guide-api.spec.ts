...
describe('GuideApi', () => {
  ...
  describe('scrollToOffset', () => {
    ...
    beforeEach(() => {
      scrollToSpy = jest.spyOn(WindowService.getWindow(), 'scrollTo').mockImplementation();
    });
    ...
    test('should scroll to top when offset is "start"', () => {
      guideApi.scrollToOffset(ScrollLocation.START);
      expect(scrollToSpy).toHaveBeenCalledWith({top: 0, behavior: 'smooth'});
    });

    test('should scroll to center of viewport when offset is "center"', () => {
      guideApi.scrollToOffset(ScrollLocation.CENTER);
      expect(scrollToSpy).toHaveBeenCalledWith({top: WindowService.getWindow().innerHeight / 2, behavior: 'smooth'});
    });
  });
});
...
function createMockElement(styleOverrides: Record<string, string> = {}): Element {
  ...
  jest.spyOn(WindowService.getWindow(), 'getComputedStyle').mockReturnValue(styles as unknown as CSSStyleDeclaration);
