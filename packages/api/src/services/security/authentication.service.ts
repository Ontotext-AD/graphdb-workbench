...
export class AuthenticationService implements Service {
  ...
  isAuthenticated(): boolean {
    const securityConfig = ServiceProvider.get(SecurityContextService).getSecurityConfig();
    return !!(securityConfig?.enabled && securityConfig?.userLoggedIn);
  }
  ...
  hasFreeAccess(): boolean | undefined {
    const securityConfig = ServiceProvider.get(SecurityContextService).getSecurityConfig();
    return securityConfig?.enabled && securityConfig?.freeAccessActive;
  }
  ...
  isSecurityEnabled(): boolean {
    return !!ServiceProvider.get(SecurityContextService).getSecurityConfig()?.enabled;
  }
}
