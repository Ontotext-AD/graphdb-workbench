...
class TestContextService extends ContextService {
  ...
  public subscribeToProperty<T>(propertyName: string, callback: (value: T | undefined) => void): () => void {
...
describe("ContextService", () => {
  ...
  test('should be called immediately when the callback function is invoked with the last value of the property', () => {
    ...
    expect(callBackFunction.mock.lastCall[0]).toEqual(valueOfTestProperty);
  });
  ...
  test('should call all callback functions with the changed value', () => {
    ...
    expect(firstCallBackFunction.mock.lastCall[0]).toEqual(valueOfTestProperty);
    expect(secondCallBackFunction.mock.lastCall[0]).toEqual(valueOfTestProperty);
  });
