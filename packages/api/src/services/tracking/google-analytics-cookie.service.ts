...
export class GoogleAnalyticsCookieService implements Service {
  ...
  private removeGoogleAnalyticsScripts() {
    const dataLayer = WindowService.getWindow().dataLayer;
    if (dataLayer) {
      // @ts-expect-error - Override the push method to prevent any further events from being pushed to the dataLayer.
