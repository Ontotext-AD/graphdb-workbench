...
export class YasguiComponentFacadeComponent implements OnDestroy {
  ...
  private delegatedPasteHandler = (event: Event) => {
    if ((event.target as HTMLElement).closest('.CodeMirror')) {
      this.codeMirrorPasteHandler();
    }
  };

  private delegatedKeyupHandler = (event: Event) => {
    const target = event.target as HTMLElement;
  ...
  private codeMirrorKeyupHandler = () => {
  ...
  private emitQueryChanged = (queryString: string) => {
  ...
  private codeMirrorPasteHandler = () => {
    ...
    ontotextYasguiElement.getQuery()
      ...
      .then((queryString: string) => this.sparqlService.addKnownPrefixes(queryString))
      .then((queryString: string) => {
        return ontotextYasguiElement.setQuery(queryString as string);
      })
      .then(() => {
        this.emitQueryChanged(JSON.stringify(queryString as string));
      })
      .catch((error) => {
  ...
  private downloadAs(query: string, infer: boolean, sameAs: boolean, authToken:string, accept: string, link: string) {
    ...
    this.rdf4jRepositoryService.downloadResultsAsFile(selectedRepository!, queryParams, accept, link)
      .then(({data, filename}: DownloadAsModel) => {
        ...
        const errorBlob = error.data as Blob | null;
