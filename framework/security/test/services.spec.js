define(["angular/security/services"],  function($provider) {
    $provider.constant("productInfo", {
        "productType":"standard","productVersion":"7.0","sesame":"2.9.0","connectors":"5.0.0"
    });

	beforeEach(module('graphdb.framework.security.services'));

	describe('$jwtAuth tests', function() {

		var $jwtAuth, $httpBackend, $rootScope, $location;

		beforeEach(inject(function (_$jwtAuth_, _$httpBackend_, _$rootScope_, _$location_) {
            ...
            httpGetAdmin = $httpBackend.when('GET', 'rest/security/user/admin').respond(200, {username: 'admin', appSettings: {}, grantedAuthorities: []});
		...
		it('$jwtAuth.autenticate should set securityEnabled to true', function(){
			...
			$jwtAuth.authenticate({username: "admin", password: "admin", expiration: 1440836388778, nonLocked: true, enabled: true, authorities: [ "ROLE_ADMIN" ]},function(){ return "1234"});
		...
		describe('$jwtAuth.canReadRepo', function(){
			it ('return correct value', function(){
				...
				$jwtAuth.hasRole = function(){return true;}
				expect($jwtAuth.canReadRepo('someLocation', 'someRepo')).toEqual(true);

				$jwtAuth.hasRole = function(){return false;}
				$jwtAuth.checkRights = function(){return true;}
				expect($jwtAuth.canReadRepo('someLocation', 'someRepo')).toEqual(true);
				$jwtAuth.checkRights = function(){return false;}
		...
		describe('$rootScope.hasPermission', function(){
			it('return correct value', function(){
				...
				$location.path = function(){return '/someUrl'}
				expect($rootScope.hasPermission()).toEqual(false);

				$location.path = function(){return '/otherUrl'}
