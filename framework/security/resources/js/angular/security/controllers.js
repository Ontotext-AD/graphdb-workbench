	...
	function() {
		...
		var getUserRoleName = function (userType) {
			switch (userType) {
				...
				case 'repoManager': return 'Repository manager';
		...
		securityCtrl.controller('UsersCtrl', ['$scope', '$http', '$modal', 'toastr', '$window', '$jwtAuth', '$timeout', 'ModalService',
			...
			$scope.removeUser = function (username) {
                ModalService.openSimpleModal({
					...
					$scope.loader = true;
					$http.delete('rest/security/user/' + encodeURIComponent(username)).
						success(function (data, status, headers, config) {
							$scope.getUsers();
		...
		securityCtrl.controller('EditUserCtrl', ['$scope', '$http', 'toastr', '$window', '$routeParams', '$timeout', '$location', '$jwtAuth', '$controller',
			...
			$scope.getUserData = function () {
				$http.get('rest/security/user/' + encodeURIComponent($scope.params.userId)).
					success(function (data, status, headers, config) {
						$scope.userData = data;
		...
		securityCtrl.controller('RolesMappingController', ['$scope', '$http', function($scope, $http) {
		    $scope.debugMapping = function(role, mapping) {
		        ...
		        $http.get('rest/roles/mapping', {params: {role : role,
		    ...
		    var loadRoles = function() {
                ...
                .success(function(data, status, headers, config) {
                    ...
                    var permissionsCount = _.map($scope.roles, function(role) {return [role, _.filter($scope.roleMappings[role]).length]});
                    $scope.roles = _.reverse(_.map(_.orderBy(permissionsCount, function(p) {return p[1]}), function(p) {return p[0]}))
