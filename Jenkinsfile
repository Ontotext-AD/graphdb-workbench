    ...
    stage('Acceptance') {
      steps {
        configFileProvider(
          ...
          // '--ansi never' and '--no-color' to supress color output that shows as garbage in Jenkins
          sh "docker-compose --ansi never build --force-rm --no-cache --parallel"
          sh "docker-compose --ansi never up --abort-on-container-exit --exit-code-from --no-color cypress-tests"
